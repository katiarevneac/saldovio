...
**Sprint 3 progress: S1, S2, S3 done. Next: S4 — Finance API requires authentication and enforces per-user ownership** (add `user_id` to `accounts`, wire the BFF shared-secret pattern so Next.js asserts the authenticated user to Finance API, negative-path test that user A cannot read/write user B's data per brief §12).
