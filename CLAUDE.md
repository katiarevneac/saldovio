...
**Process gap surfaced:** S1 and S2 were committed straight to `main`, breaking from the branch → PR → merge flow used for SAL-3 (brief §18) — not caught until after the fact, too late to undo without rewriting history. Decision: resume branch + PR for S3 onward. **Next:** mark S2 Done in Jira, then S3 — branch `feature/S3-dashboard-saves-real-transaction`, extend `prototype/` with a form that POSTs to the Finance API (replacing the hardcoded fake array), open a PR, merge.
